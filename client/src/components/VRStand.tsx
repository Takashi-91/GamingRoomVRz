...
export default function VRStand({ position = [0, 0, 0], rotation = [0, 0, 0] }: VRStandProps) {
  ...
  useFrame(() => {
    if (vrRef.current && !vrRef.current.userData.registered) {
      registerInteractiveObject(vrRef.current, "VR Headset", () => {
        console.log("VR headset interaction");
        playHit();
        
        // Animate the headset slightly
      ...
      vrRef.current.userData.registered = true;
    }
  });
  ...
  return (
    <group ref={vrRef} position={position} rotation={rotation} name="vr-stand">
      ...
      <group position={[0, 1.1, 0]}>
        ...
        {/* Headset Lenses */}
        <mesh position={[-0.05, 0, -0.01]} receiveShadow castShadow>
          <cylinderGeometry args={[0.03, 0.03, 0.06, 16]} rotation={[Math.PI/2, 0, 0]} />
      ...
      {/* VR Controllers */}
      <mesh position={[0.15, 0.5, 0]} receiveShadow castShadow>
